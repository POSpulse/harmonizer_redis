module HarmonizerRedis
  ...
  class PhraseGroup < BaseObject
    ...
    class << self
      def set_label(phrase_group_id, label)
        Redis.current.set("#{self}:#{phrase_group_id}:label", label)
      end

      def get_label(phrase_group_id)
        Redis.current.get("#{self}:#{phrase_group_id}:label")
      end

      def get_phrase_list(phrase_group_id)
        Redis.current.smembers(self.phrase_set_key(phrase_group_id))
      end

      def phrase_set_key(phrase_group_id)
        "#{self}:#{phrase_group_id}:phrase_set"
      end

      def word_set_key(phrase_group_id)
        "#{self}:#{phrase_group_id}:word_set"
      end

      #merging 2 groups with their ids
      def merge(old_group_id, new_group_id, label = nil)
